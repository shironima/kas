<div id="{{ $modalId }}" class="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 hidden">
    <div class="bg-white p-6 rounded-lg w-96">
        <h2 class="text-lg font-bold mb-4">{{ $method == 'POST' ? 'Tambah' : 'Edit' }} Pengeluaran</h2>
        ...
        <form method="POST" action="{{ $action }}">
            ...
            <div class="flex justify-end space-x-2">
                ...
                <button type="submit" class="bg-red-600 text-white px-3 py-2 rounded-md">Simpan</button>
            </div>
        </form>
    </div>
</div>
